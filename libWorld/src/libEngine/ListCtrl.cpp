...
void ListCtrl::Draw( SHORT x, SHORT y )
{
    ...
    for(size_t i = m_indexStart; i < m_vecItems.size(); i++)
    {
        ...
        // 绘制背景;
        DrawSkin(m_dwItemSkinID, x + m_posX, y + m_posY + (int)i * m_nItemHeight, m_width, m_nItemHeight);
        m_vecItems[i]->Draw(x + m_posX, y + m_posY + (int)i * m_nItemHeight, m_width, m_nItemHeight);
    }
    ...
    if(m_indexSelected != -1)
    {
        DrawSkin(m_dwItemSelectedSkinID, 
            x + m_posX, 
            y + m_posY + (int)(m_indexSelected - m_indexStart) * m_nItemHeight, 
            m_width, 
            m_nItemHeight);
    }
    ...
    if(m_indexHover != -1)
    {
        DrawSkin(m_dwItemHoverSkinID, 
            x + m_posX, 
            y + m_posY + (int)(m_indexHover - m_indexStart) * m_nItemHeight, 
            m_width, 
            m_nItemHeight);
    }
...
BOOL ListCtrl::MouseMsgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    int _index = (pt.y - sPosY) / m_nItemHeight;
    _index = _index + (int)m_indexStart;

    if(_index >= (int)m_vecItems.size())
    {
        m_indexHover = -1;
        return TRUE;
    }
