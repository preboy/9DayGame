...
namespace LibEngine
{
	...
	CMD5::MD5VAL CMD5::GetMD5Code(const char * str, unsigned int size)
	{
		if(size==0)
		{
			size=strlen(str);
	...
	//MD5文件摘要
	CMD5::MD5VAL CMD5::GetMD5FileCode(const char* szFileName)
	{
		FILE* fpin = NULL;
		MD5VAL val={0x0,0x0,0x0,0x0};
		if ( 0 != fopen_s(&fpin, szFileName, "r" ) )
		{
			return val;
		}
		if(!Buffer)
	...
	void	CMD5::GetMD5String(char* szValue, const char * str, unsigned int size )
	{
		MD5VAL	mValue = GetMD5Code(str, size );
		sprintf_s( szValue, 32, "%X%X%X%X", mValue.a, mValue.b, mValue.c, mValue.d );
	}

	void	CMD5::GetMD5FileString(char* szValue, const char* szFileName)
	{
		MD5VAL	mValue = GetMD5FileCode( szFileName );
		sprintf_s( szValue, 32, "%X%X%X%X", mValue.a, mValue.b, mValue.c, mValue.d );
	}
