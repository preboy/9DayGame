#include "stdafx.h"
#include "LibEngine/LuaScriptBase.h"
...
namespace LibEngine
{
    ...
    bool LuaScriptBase::CallFunction(const char* funcName, LuaFunction& funObj, int expectReturnCount)
    {
        ...
        int nRet = lua_pcall(m_luaState, funObj.vecParam.size(), expectReturnCount, 0);
        if(LUA_OK != nRet)
        {
        ...
        for(int i=1; i<=expectReturnCount; i++)
        {
            ...
            {
                funObj.vecRet.push_back(luaParam(lua_toboolean(m_luaState, i)));
            }
